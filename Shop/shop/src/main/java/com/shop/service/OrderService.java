...
public class OrderService 
{
	...
	public boolean validateOrder(Long orderId, String email) // 현재 로그인한 사용자와 주문 데이터를 생성한 사용자가 같은지 확인
	{
	...
	{
		Order order = orderRepository.findById(orderId).orElseThrow(EntityNotFoundException::new);
		order.cancelOrder(); // 주문 취소 상태로 변경하면 변경 감지 기능에 의해 트랜잭션이 끝날 때 업데이트 쿼리가 실행
	}
